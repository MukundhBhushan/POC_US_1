...
app.get('/carpenter/getrepairs',(req,res)=>{
    dbRepair.repairs.find({"skill":"carpenter"}, (err, docs)=>{
    ...
    dbRepair.repairs.find({"skill":"carpenter"}, (err, docs)=>{
        ...
        keys.forEach(key => {
            ...
            dbRepair.repairs.findAndModify({
                query: { _id: mongojs.ObjectId(key) },
                update: { $set: { accName: arr[2]},$push:{prices:parseInt(arr[1])},$push:{NameWork:arr[0]}},
                new:false
...
app.get('/plumber/getrepairs',(req,res)=>{
    dbRepair.repairs.find({"skill":"plumber"}, (err, docs)=>{
        ...
        keys.forEach(key => {
            ...
            dbRepair.repairs.findAndModify({
                query: { _id: mongojs.ObjectId(key) },
                update: { $set: { accName: arr[1]},$push:{prices:parseInt(arr[0])},$push:{prices:(arr[2])}},
                new:false
            },function (err, doc, lastErrorObject) {
                console.log(err)
            })          
...
app.get('/seeList/carpenter',(req,res)=>{
    dbRepair.repairs.aggregate({$match:{
        "skill":"carpenter"
    }},
